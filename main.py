...
import wandb
from inference import (get_accuracy_imagenet_real, get_correct_prob,
                       get_pug_imagenet, run_imagenethard, run_imagenetx,
                       run_robustness)
from transforms import CropScale, CropShift
from utils import (ImageFolderWithPaths, RealLabelsImagenet, get_world_size,
...
def main(args):
    ...
    if args.experiment == "imagenetx" or args.experiment == "pug_imagenet":
        ...
        def apply_transforms(examples):
            ...
            examples["image"] = [
                transform_val(image) for image in examples["image"]
            ]
            return examples
    ...
    if args.experiment == "imagenethard":
    ...
    if (args.experiment == "scale" or args.experiment == "shift_xy"
        ...
        return get_correct_prob(data_loader_val, model, device)
    elif args.experiment == "robustness":
        return run_robustness(model, args.data_path, transform_val)
    elif args.experiment == "imagenetx":
        return run_imagenetx(data_loader_val, model, device, model_name)
    elif args.experiment == "imagenethard":
        return run_imagenethard(data_loader_val, model, device)
    elif args.experiment == "pug_imagenet":
        return get_pug_imagenet(model, args.data_path, transform_val)
        ...
        real_labels_imagenet = RealLabelsImagenet(filenames,
                                                  real_json="./real.json")
        return get_accuracy_imagenet_real(data_loader_val, model, device,
